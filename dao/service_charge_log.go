...
func (this *DataBaseAccessObject) GetServiceChargeLogPage(start, pageSize int, anchorNodeId uint) ([]*ServiceChargeLog, error) {
	result := make([]*ServiceChargeLog, 0)
	db := this.db.Table((&ServiceChargeLog{}).TableName()).
		Where("status=?", 1)
	if anchorNodeId != 0 {
		db = db.Where("anchor_node_id=?", anchorNodeId)
	}
